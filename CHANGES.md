...
- [FIX] C API の `fmp4_segment_demuxer_handle_media_segment` が、変換できないサンプルエントリーで `MP4_ERROR_UNSUPPORTED` を返したときに内部の `Fmp4SegmentDemuxer` の状態を変えないようにする
  ...
  - @voluntas
- [FIX] `Fmp4SegmentDemuxer::handle_media_segment` が、`data_offset` のない 2 つ目以降の `trun` のサンプルを、直前の `trun` のデータの直後から取り出すようにする
  - これまでは `traf` の基準位置から取り出していたため、エラーにならずに誤ったバイト列をサンプルデータとして返していた
