...
//! - RFC 4648: base16 (Section 4 の文字集合)
//! - RFC 6184: profile-level-id (Section 8.1 の 3 バイトの base16 表現)
//!
//! # NAL バイト列の契約
//!
...
impl H264ProfileLevelId {
    ...
    /// `A-F` と `a-f` の両方を受理する。profile や level の意味検証は行わず、
    /// 任意の 3 バイトを保持する。
    ///
    /// # エラー条件
    ///
    ...
    /// 3 バイトを先頭ゼロを省略しない 6 桁の小文字 hex へ変換する
    ///
