...
impl FullBoxFlags {
    ...
    /// この扱いによって、素朴に `1 << x.0` と書いた場合に起きる debug ビルドでのシフトオーバーフローによるパニックと release ビルドでのラップを回避する。
    ///
    /// 同じビット位置が複数回渡された場合は OR で合成される（冪等）。
    /// 素朴に加算で畳み込むと u32 加算オーバーフローでパニックする可能性があるため、内部では OR 畳み込みを行う。
    ...
    /// この扱いによって、素朴に `1 << i` と書いた場合に起きる debug ビルドでのシフトオーバーフローによるパニックと release ビルドでのラップによる誤判定を回避する。
