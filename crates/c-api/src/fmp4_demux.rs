...
pub struct Mp4Fmp4Demuxer {
    inner: Fmp4Demuxer,
    /// キャッシュ済みのトラック情報。`None` は未初期化または未取得を表す。
    tracks_cache: Option<Vec<Mp4Fmp4TrackInfo>>,
...
#[unsafe(no_mangle)]
pub extern "C" fn mp4_fmp4_demuxer_new() -> *mut Mp4Fmp4Demuxer {
    Box::into_raw(Box::new(Mp4Fmp4Demuxer {
        inner: Fmp4Demuxer::new(),
        tracks_cache: None,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mp4_fmp4_demuxer_free(demuxer: *mut Mp4Fmp4Demuxer) {
    if !demuxer.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mp4_fmp4_demuxer_get_last_error(
    demuxer: *const Mp4Fmp4Demuxer,
) -> *const c_char {
    if demuxer.is_null() {
        return c"".as_ptr();
    }
...
/// - その他のエラー: 処理に失敗した
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mp4_fmp4_demuxer_handle_init_segment(
    ...
    match demuxer.inner.handle_init_segment(data) {
        ...
        Err(e) => {
            demuxer.set_last_error(&format!("[mp4_fmp4_demuxer_handle_init_segment] {e}"));
            e.into()
        }
    }
}
...
/// - その他のエラー: 取得に失敗した
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mp4_fmp4_demuxer_get_tracks(
    demuxer: *mut Mp4Fmp4Demuxer,
    out_tracks: *mut *const Mp4Fmp4TrackInfo,
    out_count: *mut u32,
) -> Mp4Error {
    ...
    if demuxer.tracks_cache.is_none() {
        match demuxer.inner.tracks() {
            ...
            Err(e) => {
                ...
                demuxer.set_last_error(&format!("[mp4_fmp4_demuxer_get_tracks] {e}"));
                return e.into();
            }
        }
    }
    ...
    let count = match u32::try_from(cached.len()) {
        ...
        Err(_) => {
            ...
            demuxer.set_last_error("[mp4_fmp4_demuxer_get_tracks] track count exceeds u32::MAX");
...
/// - その他のエラー: 処理に失敗した
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mp4_fmp4_demuxer_handle_media_segment(
    demuxer: *mut Mp4Fmp4Demuxer,
    data: *const u8,
    size: u32,
    out_samples: *mut *mut Mp4Fmp4DemuxSample,
    out_count: *mut u32,
) -> Mp4Error {
    ...
    match demuxer.inner.handle_media_segment(data) {
        Ok(samples) => {
            let mut c_samples: Vec<Mp4Fmp4DemuxSample> = Vec::new();
            for s in &samples {
                let data_size = match u32::try_from(s.data_size) {
                    ...
                    Err(_) => {
                        ...
                        demuxer.set_last_error(
                            "[mp4_fmp4_demuxer_handle_media_segment] data_size exceeds u32::MAX",
                        );
                        return Mp4Error::MP4_ERROR_OTHER;
                    }
                };
                c_samples.push(Mp4Fmp4DemuxSample {
                    track_id: s.track_id,
            ...
            let count = match u32::try_from(c_samples.len()) {
                ...
                Err(_) => {
                    ...
                    demuxer.set_last_error(
                        "[mp4_fmp4_demuxer_handle_media_segment] sample count exceeds u32::MAX",
                    );
                    return Mp4Error::MP4_ERROR_OTHER;
                }
            };
...
/// - `count`: サンプル数
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mp4_fmp4_demuxer_free_samples(
    samples: *mut Mp4Fmp4DemuxSample,
    count: u32,
) {
    if samples.is_null() {
        return;
