...
- `AudioSampleEntryFields::channelcount` = `output_channel_count`
- `OpusBox::unknown_boxes` = 空 `Vec`
- `DopsBox` の 4 フィールド = `OpusSampleEntryConfig` の対応する値

現行 `DopsBox` が固定する `ChannelMappingFamily = 0` は mono / stereo の family なので、`output_channel_count` は 1 または 2 だけを受理する。0 または 3 以上は `ErrorKind::InvalidInput` とし、対応していない multistream の box を生成しない。
...
- `tests/test_bitstream_opus.rs` に mono / stereo の構築結果と、チャンネル数 0 / 3 の拒否を追加する
...
- `bitstream::opus::OpusSampleEntryConfig` と `build_opus_box` が公開される
