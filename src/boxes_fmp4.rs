    ...
    fn encode(&self, buf: &mut [u8]) -> Result<usize> {
        ...
        if self.full_box_version() == 1 {
            ...
            let ept = u32::try_from(self.earliest_presentation_time).map_err(|_| {
                crate::Error::invalid_data(
                    "SidxBox earliest_presentation_time exceeds u32::MAX for version 0",
                )
            })?;
    ...
    fn encode(&self, buf: &mut [u8]) -> Result<usize> {
        ...
        for entry in &self.entries {
            if version == 1 {
                ...
                let mo = u32::try_from(entry.moof_offset).map_err(|_| {
                    crate::Error::invalid_data(
...
fn encode_variable_uint(value: u32, byte_count: u8, buf: &mut [u8]) -> Result<usize> {
    ...
    // インデックス範囲外パニックを防ぐために match の直前で長さを検査する
    // （4 バイトのアームは value.encode(buf) 側でも同じ検査が走るが、
    // 全アームで検査経路を統一するため二重検査を許容する）
