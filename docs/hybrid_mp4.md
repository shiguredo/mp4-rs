...
| 形式 | 録画中の構造 | 完成後の見え方 | 主な性質 |
| --- | --- | --- | --- |
| 標準 MP4 | `ftyp` + (予約領域) + `mdat` + 終端の `moov` | そのまま標準 MP4 | 互換性は高いが、`moov` が無いと再生困難 |
| fMP4 | `ftyp` + 不完全な `moov` + `moof` / `mdat` の列 | そのまま fMP4 | 途中停止に強いが、編集ソフト等での互換性が低いことがある |
| Hybrid MP4 | 録画中は fMP4 と同様 | 終了時に標準 MP4 相当へ変換 | 録画中の耐性と完成後の互換性を両立する |
...
C API では `mp4_file_muxer_advance_position()` が同等の役割を持つ。呼び出し例は Rust API と同じ順序で組み立てればよい。
